...
TEST(TaskGraph, SingleTask)
{
    ...
    auto task = g.addTask<int>(
        "task",
        [&](std::span<const int> numbers, std::pmr::memory_resource* pMemoryResource) {
...
TEST(TaskGraph, TaskChain)
{
    ...
    auto task4 = g.addTask<std::pair<int, int>>(
        "task4",
        [&](std::span<const std::pair<int, int>> numbers, std::pmr::memory_resource* pMemoryResource) {
...
TEST(TaskGraph, StaticData)
{
    ...
    auto task = g.addTask<int, StaticData>(
        ...
        [&](std::span<const int> numbers, const StaticData* pStaticData, std::pmr::memory_resource* pMemoryResource) {
            for (const int number : numbers)
...
TEST(TaskGraph, CachedStaticData)
{
    ...
    auto task = g.addTask<int, StaticDataCollection>(
        ...
        [&](std::span<const int> numbers, const StaticDataCollection* pStaticDataCollection, std::pmr::memory_resource* pMemoryResource) {
            for (const int number : numbers)
