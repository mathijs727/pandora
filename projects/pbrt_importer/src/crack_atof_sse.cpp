...
float crack_atof_sse(std::string_view string)
{
    ...
    //if (pointOffset == 16) //|| pBegin[pointOffset] == 'e')
    //    return fallback(string);

    // TODO: support float without "." (= int)
    ...
    sum = _mm_add_ps(sum, _mm_mul_ps(powers_1, chars_ps_1));
    const __m128 powers_2 = _mm_loadu_ps(powerLUT.data() + startOffset + 4);
    sum = _mm_add_ps(sum, _mm_mul_ps(powers_2, chars_ps_2));
    const __m128 powers_3 = _mm_loadu_ps(powerLUT.data() + startOffset + 8);
    sum = _mm_add_ps(sum, _mm_mul_ps(powers_3, chars_ps_3));
    const __m128 powers_4 = _mm_loadu_ps(powerLUT.data() + startOffset + 12);
    sum = _mm_add_ps(sum, _mm_mul_ps(powers_4, chars_ps_4));
    ...
    _mm_store_ps(sumData, sum);

    return sign * sumData[0];
}