...
Group GroupBuilder::build(std::vector<std::string> nameHierarchy) const
{
    ...
    for (auto& [name, child] : m_children) {
        ...
        auto group = std::make_unique<Group>(child->build(childNameHierarchy));
    ...
    for (auto& name : m_counters) {
        ...
        valueMap[name] = std::make_unique<Counter>(std::move(identifier));
    }
