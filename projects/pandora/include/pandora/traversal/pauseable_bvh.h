...
struct is_pauseable_leaf_obj {
	...
	static constexpr bool value = has_member_function_intersect<T, bool, boost::mpl::vector<Ray&, SurfaceInteraction&, const UserState&, PauseableBVHInsertHandle>, boost::function_types::const_qualified>::value;
};
...
public:
	...
	virtual bool intersect(Ray& ray, SurfaceInteraction& si, const UserState& userState) const = 0;
	virtual bool intersect(Ray& ray, SurfaceInteraction& si, const UserState& userState, PauseableBVHInsertHandle handle) const = 0;
};
