...
void fullTest(VoxelGrid& voxelGrid, const VoxelizeTriangleData& data)
{
	...
	for (int z = data.tBoundsMinVoxel.z; z < std::min(data.tBoundsMaxVoxel.z, data.gridResolution.z); z++) {
		for (int y = data.tBoundsMinVoxel.y; y < std::min(data.tBoundsMaxVoxel.y, data.gridResolution.y); y++) {
			for (int x = data.tBoundsMinVoxel.x; x < std::min(data.tBoundsMaxVoxel.x, data.gridResolution.x); x++) {
				// Intersection test
...
void meshToVoxelGridNaive(VoxelGrid& voxelGrid, const Bounds& gridBounds, const TriangleMesh& mesh)
{
	...
	data.gridResolution = glm::ivec3(voxelGrid.resolution());

	// World space extent of a voxel
	data.delta_p = scale / glm::vec3(voxelGrid.resolution());
	...
	for (int t = 0; t < triangles.size(); t++) {
		...
		data.tBoundsMinVoxel = glm::min(worldToVoxel(data.tBoundsMin), data.gridResolution - 1); // Fix for triangles on the border of the voxel grid
