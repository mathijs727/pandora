...
IF (NOT PANDORA_ISPC_EXECUTABLE)
  ...
  FIND_PROGRAM(PANDORA_ISPC_EXECUTABLE ispc PATHS ${ISPC_DIR_HINT} DOC "Path to the ISPC executable.")
  IF (NOT PANDORA_ISPC_EXECUTABLE)
    MESSAGE(FATAL_ERROR "Intel SPMD Compiler (ISPC) not found. Please install ISPC.")
  ELSE()
    MESSAGE(STATUS "Found Intel SPMD Compiler (ISPC): ${PANDORA_ISPC_EXECUTABLE}")
  ENDIF()
ENDIF()

# check ISPC version
EXECUTE_PROCESS(COMMAND ${PANDORA_ISPC_EXECUTABLE} --version OUTPUT_VARIABLE ISPC_OUTPUT)
...
GET_FILENAME_COMPONENT(ISPC_DIR ${PANDORA_ISPC_EXECUTABLE} PATH)

SET(PANDORA_ISPC_ADDRESSING 32 CACHE INT "32vs64 bit addressing in ispc")
MARK_AS_ADVANCED(PANDORA_ISPC_ADDRESSING)
...
MACRO (ISPC_COMPILE)
  ...
  FOREACH(src ${ARGN})
	...
	IF (ISPC_TARGET_ARGS STREQUAL "")
	  SET(ISPC_TARGET_ARGS "host")
	ENDIF()

    ADD_CUSTOM_COMMAND(
      OUTPUT ${results} ${ISPC_TARGET_DIR}/${fname}_ispc.h
      COMMAND ${CMAKE_COMMAND} -E make_directory ${outdir}
      COMMAND ${PANDORA_ISPC_EXECUTABLE}
      -I ${CMAKE_CURRENT_SOURCE_DIR}
      ${ISPC_INCLUDE_DIR_PARMS}
      ${ISPC_DEFINITIONS}
      --arch=${ISPC_ARCHITECTURE}
      --addressing=${PANDORA_ISPC_ADDRESSING}
      ${ISPC_OPT_FLAGS}
...
MACRO (ADD_ISPC_LIBRARY name type)
   ...
   FOREACH(src ${ARGN})
    ...
    ELSE ()
	  MESSAGE(WARNING "Warning: ignoring source file ${src} for ISPC compilation")
